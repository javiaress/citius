...
class SSM(nn.Module):
    ...
    def forward(self, x, time_prev):
        ...
        for i in range(seq):
            ...
            dt_i = time_prev[:, i, 0].unsqueeze(-1)                     # (batch, 1)
            dt = dt_i.expand(-1, self.d_inner)                     # (batch, d_inner)
            #print(f"dt shape: {dt.shape}\n\n")
            ...
            if i == seq - 1:
                hidden_seq = torch.stack(hidden_previos)  # (seq, batch, d_inner, d_state)
                hidden_seq = hidden_seq.permute(1, 0, 2, 3)  # (batch, seq, d_inner, d_state)
...
class Modelo_ind(nn.Module):
    ...
    def forward(self, x):
        ...
        out, _ = self.ssm(x_attn, time_prev)
